...
export class KubernetesCluster
{
    ...
    async processIngresses()
    {
        ...
        for (let ingress of ingresses)
        {
            try
            {
                if (ingress.metadata?.name && (ingress.metadata?.annotations || {})['kubernetes.io/ingress.class'] == 'sapienafrica')
                {
                        ...
                        if (host)
                        {
                            ...
                            for (let path of rule.http?.paths || [])
                            {
                                ...
                                if (service?.name)
                                {
                                    ...
                                    if (serviceInfo.spec?.clusterIP && path.path && path.pathType && service.port.number)
                                    {
